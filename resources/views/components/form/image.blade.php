...
<div class="{{ $class_group ?? null }}" @if (!$show) style="display: none" @endif>
    ...
    @if ($multiple)
        <input class="form-control" type="file" id="{{ $field_name }}" name="{{ $field_name }}"
            {{ isset($disabled) && $disabled ? 'disabled' : '' }}
            {{ isset($required) && $required && !isset($src) ? 'required' : '' }} accept="{{ $accept ?? 'image/*' }}"
            multiple>
    @else
        <input class="form-control" type="file" id="{{ $field_name }}" name="{{ $field_name }}"
            {{ isset($disabled) && $disabled ? 'disabled' : '' }}
    ...
    @if (isset($src) and $src != '//')
        <div class="mt-4">
            <img src="{{ $src }}" height="200rem" width="200rem" />
        </div>
    @endif
</div>
